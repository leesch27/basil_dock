...
In addition, on Windows Docker is required to handle packages with no windows serialization option (FPocket), this can be installed from https://docs.docker.com/desktop/setup/install/windows-install/
...
\*If running the jupyter notebooks or Streamlit on Windows, ensure Docker Desktop is installed and running. Once the program has been launched the Docker Desktop window can be closed, and it will remain active in the background.
